...
8.  On this page, we'll be specifying a variety of settings for our infrastructure.
    ...
    -   **Guest Customization** - Guest customization allows for the modification of certain settings at boot. Linux OSes use ``Cloud Init``, while Windows OSs use ``Sysprep``. 
        ...
        Take note of the "@@{vm_password}@@" text. In NCM Self Service the "@@{" and "}@@" characters represent a macro. At runtime, NCM Self Service will
        automatically \"patch\" or substitute in the proper value(s) when it encounters a macro. A macro could represent a system
        defined value, a VM property, or (as it does in this case) a runtime variable. Later in this lab we\'ll create a runtime
...
Variables allow extensibility of Blueprints, meaning a single Blueprint can be used for multiple purposes and environments depending on the
configuration of its variables. Variables can either be static values saved as part of the Blueprint or they can be specified at **Runtime**
(when the Blueprint is launched), as they will in this case.

In a Single VM blueprint, variables can be accessed by clicking the **App variables** button near the top. By default, variables are stored
as a **String**, however additional **Data Types** (Integer, Multi-line String, Date, Time, and Date Time) are all possible. Any of these data
types can be optionally set as **Secret**, which will mask its value and is ideal for variables such as passwords. There are also more advanced
**Input Types** (versus the default **Simple**), however these are outside the scope of this lab.

Variables can be used in scripts executed against objects using the **@@{variable_name}@@** construct (called a macro). Calm will expand and
replace the variable with the appropriate value before sending to the VM.
...
5.  To view the current memory on CentOS, run the command **free -h**.
    ...
    ![](images/22_centos_memory_before.png)
...
10. Back in the **VM Console**, run the same command from earlier to view the updated memory, and note that it has increased by 2 GiB.
    ...
    ![](images/26_centos_memory_after.png)
...
Now that we know we have a good blueprint, lets publish it to the Marketplace.
...
3.  Click the **Publish** button, and enter the following:
    ...
    ![](images/28_centos_publish_bp.png)
...
6.  Click **Approve**.
    ...
    ![](images/29_centos_approve_bp.png)
...
7.  Select **Marketplace Blueprints** at the top of the page, and enter
    ...
    ![](images/30_centos_marketplace_bp.png)
...
5.  Under **Credential**, click **+** and enter the following:
    ...
    ![](images/32_centos_project_creds.png)
...
6.  Under **VM Configuration** expand **Linux**, and enter the
    ...
    ![](images/33_centos_project_vmconfig.png)
...
3.  Select your *intials***\_CentOS_IaaS** blueprint, and click
    **Launch** from the Marletplace.
    ...
    ![](images/31_centos_marketplace_launch_bp.png)
...
What are the key things you should know about **NCM Self Service** and  **Single VM Blueprints**?

-   NCM Self Service provides application and infrastructure automation natively within Prism, turning complex, week long ticketing
...
-   Common day 2 operations, like snapshotting, restoring, cloning, and updating the infrastructure can all be done by end users directly
    within Calm.
