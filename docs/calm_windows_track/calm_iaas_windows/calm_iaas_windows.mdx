...
**In this lab you\'ll create a \"Single VM Blueprint\" based on Windows,
launch the blueprint, and manage the resulting application.**
...
6.  Note the following fields on the **VM Details** page:
    ...
    -   **Cloud** - The cloud we're deploying the infrastructure on is Nutanix so it should be left as **Nutanix**.
    -   **Operating System** - Windows (The type of OS we\'re deploying)

    ![Windows 2016 VM Details](images/5_windows_2.png)
...
8.  On this page, we'll be specifying a variety of settings for our infrastructure.
    ...
    -   **General Configuration**
        ...
        ![Windows 2016 VM Configuration - General Configuration](images/7_windows_3.png){.align-center}
    ...
    -   Select the **Guest Customization**, and then paste in the following script. Leave  the Install Type (Prepared) and Join a Domain (un-checked) as defaults.
    ...
    -   Windows 2016
        ...
        ![Windows 2016 Sysprep](images/9_windows_4.png){.align-center}

        :::note

        Take note of the ``@@{vm_password}@@`` text. In Calm the ``@@{\"and \"}@@`` characters represent a macro. At runtime, Calm willautomatically ``patch`` or substitute in the proper value(s) when it encounters a macro. A macro could represent a system
        defined value, a VM property, or (as it does in this case) a runtime variable. Later in this lab we\'ll create a runtime variable with the name ``vm_password``.
    ...
    -   **Disks** - A disk is the storage of the VM or infrastructure
        ...
        -   **Bus Type** - The bus type of the disk, this can be left asdefault (**SCSI**).
        -   **Operation** - How the disk will be sourced. "Allocate onStorage Container" is used for blank disks. We're going to keep the default, **Clone from Image Service**, as we're using a pre-defined image.
        ...
        ![Windows 2016 VM Configuration - Disks](images/11_windows_5.png)
    ...
    -   **Categories** - Categories span several different products and solutions within the Nutanix portfolio. They enable you to set security policies, protection policies, alert policies, and playbooks. Simply choose the categories corresponding to the workload, and all of these policies will automatically be
        applied. In this lab however, we're going to leave this field **blank**.

        ![VM Configuration - Boot Configuration, vGPUs, and Categories](images/12_boot_gpu_cat.png)

    -   **NICs** - Network adapters allow communication to and from yourvirtual machine. We\'ll be adding a single NIC by clicking the
        **blue plus**, then selecting **Primary** in the dropdown, and
        selecting the **Dynamic** radio button.

        ![VM Configuration - NICs](images/13_vm_nic.png)

    -   **Serial Ports** - Whether or not the VM needs a virtual serial
        port. We\'ll leave the default of **none**.

        ![VM Configuration - Serial Ports](images/14_serial.png)

9.  At the bottom of the page, click the blue **Save** button. It is expected to have a single error about an incorrect macro due to our
    Guest Customization containing "vm_password". If you have additional errors, please be sure to resolve them before continuing to the next section.

    ![Blueprint Save - Error](images/15_error.png)
...
2.  Click **Create**, where you\'ll be redirectly the application page.
...
5.  To view the current memory on Windows, open a **Command Prompt**,
    and run **systeminfo \| findstr Memory**. Take note of the current
    memory allocated to your VM.

    ![Windows Memory - Before Update](images/23_windows_memory_before.png){.align-center}

6.  Navigate back to the application page of Calm, and click the
    **Update** button in the upper right. On the page that appears,
    increase the **Memory (GiB)** field by 2 GiB (For Windows, 8 GiB).

7.  Click the blue **Update** button in the lower left.

8.  Validate that the memory field has been increased by 2 GiB, and
    click **Confirm**.

    ![Windows Memory - Confirm Change](images/25_windows_confirm.png){.align-center}

9.  In the **Audit** tab of Calm, wait for the **App Update** action to
    complete.

10. Back in the **VM Console**, run the same command from earlier to
    view the updated memory, and note that it has increased by 2 GiB.

    ![Windows Memory - After Update](images/27_windows_memory_after.png){.align-center}
...
2.  Click your *Initials***-Windows-IaaS** blueprint.
...
3.  Click the **Publish** button, and enter the following:
    ...
    ![Windows Publish Blueprint](images/28_windows_publish_bp.png){.align-center}
...
6.  Click **Approve**.
    ...
    ![Windows Approve Blueprint](images/29_windows_approve_bp.png){.align-center}
...
7.  Select **Marketplace Blueprints** at the top of the page, and enter
    ...
    ![Windows Marketplace Blueprint](images/30_windows_marketplace_bp.png){.align-center}
...
5.  Under **Credential**, click ``plus`` sign and enter the following details:
    ...
    ![Windows Project Credential](images/32_windows_project_creds.png){.align-center}

6.  Under **VM Configuration** expand **Windows**, and enter the following:
    ...
    ![Windows Project VM Config](images/33_windows_project_vmconfig.png){.align-center}

7.  Click **Save**.

## Launching the Blueprint from the Marketplace

1.  Select ![mktmgr-icon](../images/marketplacemanager_icon.png) **Marketplace Manager** in the left hand toolbar to view and manage Marketplace Blueprints.

2.  Enter your *initials* in the search bar, and you should see your blueprint listed.

3.  Select your *intials***\_Windows_IaaS** blueprint, and click **Launch** from the Marletplace.

    ![Windows Marketplace Launch Blueprint](images/31_windows_marketplace_launch_bp.png){.align-center}
...
-   Nutanix Calm provides application and infrastructure automation natively within Prism, turning complex, week long ticketing
    processes, into one-click self service provisioning.
-   While Multi VM blueprints enable the provisioning and lifecycle management of complex, multi-tiered applications, Single VM
    blueprints allows IT to provide Infrastructure-as-a-Service for their end users.
